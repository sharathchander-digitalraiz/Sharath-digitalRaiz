...
exports.add_culvert = async(req,res)=>
{
    ...
    const qr_code_image_path='uploads/qr-images/culvert/qr_codes/';
    var template_image_path='uploads/qr-images/culvert/templates/';
...
function (err)
{
  loadImage("./culvert_template.png").then((image) => 
   {
...
{
const data = await culvert.find({status:'Active'}).exec();
...
var type='';
var check="";
if(culvert_type_data.name=='Normal Flow')
{
  type='green';
}
else if(culvert_type_data.name=="Water OverFlow")
{
...
else if(culvert_type_data.name=="Drinage blockage")
{
   type='red';
}
...
// current minutes
  ...
  // current seconds
    ...
    const cuv_issue = await culvert_issues.updateOne({_id:culvert_issue_id},{
    culvert_id:culvert_id,
...
{
  responseObject = {'success':false,login: true,message:longitude1+'&&'+latittude1};
  res.status(400).json(responseObject);
}
