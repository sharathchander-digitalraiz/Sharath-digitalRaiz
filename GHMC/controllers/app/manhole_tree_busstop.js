...
exports.createmanhole_tree = async (req, res) => {
  ...
  const qr_code_image_path = "uploads/qr-images/man_hole/qr_codes/";
  var template_image_path = "uploads/qr-images/man_hole/templates/";
  ...
  QRCode.toFile(
    ...
    { version: 10 },
    function (err) {
      loadImage("./Manhole.jpg").then((image) => {
      ...
      loadImage(qr_codes).then((image) => {
        ...
        context.fillText(man_hole_name, 435, 807);

        //  //vehicle type
        context.fillStyle = "#000";
        context.font = "28px cambria";
        context.textAlign = "left";
        context.fillText(type, 435, 865);
  ...
  const unique_noss = new unique_nos({
    unique_no: unique_no,
    user_id: user_id,
    type_db: "man_hole_tree_busstops"
  });
...
exports.generate = async (req, res) => {
  ...
  const checkstring = await manhole_tree
...
exports.getallmanhole_tree = async (req, res) => {
  const alldata = await manhole_tree.find({ status: "Active" }).exec();
  return res.status(200).json({ status: true, data: alldata });
};

exports.editmanhole_tree = async (req, res) => {
  const id = req.body.id;
  await manhole_tree.findOne({ _id: id }, (err, data) => {
    if (err) {
      return res
        .status(404)
...
exports.updatemanhole_tree = async (req, res) => {
  const id = req.body.id;
  const {
    zones_id,
    circles_id,
  ...
  if (req.files.length > 0) {
    ...
    var compleximg = await manhole_tree
  ...
  const qr_code_image_path = "uploads/qr-images/man_hole/qr_codes/";
  var template_image_path = "uploads/qr-images/man_hole/templates/";
  const unique_nos = await manhole_tree
  ...
  await manhole_tree.findByIdAndUpdate(
    { _id: id },
    { ...updateRecords },
    (err, data) => {
      //  console.log(data);
...
exports.deletemanhole_tree = async (req, res) => {
...
exports.uploadmanhole_tree = multer({ storage: storage });

exports.manhoelScan_cronjob = async (req, res) => {
  const manholeRows = await manhole_tree.find({}).exec();
  const docs = [];
