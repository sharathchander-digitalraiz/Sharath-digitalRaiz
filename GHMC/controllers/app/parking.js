...
exports.createparking = async (req, res) => {
  ...
  const arearow = await area
    ...
    .exec();
  // console.log(arearow);
  const qr_code_image_path = "uploads/qr-images/parking/qr_codes/";
  var template_image_path = "uploads/qr-images/parking/templates/";
  ...
  QRCode.toFile(
    ...
    { version: 10 },
    function (err) {
      loadImage("./Parking.jpg").then((image) => {
      ...
      loadImage(qr_codes).then((image) => {
        ...
        context.fillText(parking_name, 435, 807);

        //  //vehicle type
        context.fillStyle = "#000";
        context.font = "28px cambria";
        context.textAlign = "left";
        context.fillText(owner_name, 435, 865);
...
exports.generate = async (req, res) => {
  ...
  const checkstring = await parking
...
exports.getallparking = async (req, res) => {
  const alldata = await parking.find({ status: "Active" }).exec();
  return res.status(200).json({ status: true, data: alldata });
};

exports.editparking = async (req, res) => {
  const id = req.body.id;
  await parking.findOne({ _id: id }, (err, data) => {
    if (err) {
      return res
        .status(404)
        .json({ status: false, message: "Parking data not exists" });
    } else {
...
exports.updateparking = async (req, res) => {
  ...
  if (!id) {
    return res
      .status(404)
      .json({ status: false, message: "Parking id is required" });
  }
  ...
  if (req.files.length > 0) {
    ...
    var compleximg = await parking.findOne({ _id: id }, { image: 1 }).exec();
  ...
  const qr_code_image_path = "uploads/qr-images/parking/qr_codes/";
  var template_image_path = "uploads/qr-images/parking/templates/";
  const unique_nos = await parking
  ...
  await parking.findByIdAndUpdate(
    ...
    (err, data) => {
      ...
      if (err) {
        return res
          .status(404)
          .json({ status: false, message: "Sorry unable to update parking" });
      } else if (data == null) {
...
exports.upload_parking = multer({ storage: storage });

// parking scan cron job
exports.parking_cronjob = async function (req, res) {
  try {
    const showParking = await parking.find({}, {});
    ...
    */
    ...
    showParking.forEach(async function (item) {
      let copyDocuments = {
        date: formattedDate,
      ...
      let result = await parking_operation(copyDocuments).save();
    });
...
exports.Parking_report_search = async (req, res) => {
  const {
    user_id,
    zones_id,
    circles_id,
...
async function alldata(issueObject, date1, date2, dates) {
  ...
  const dbData = await parking_operation
    .find(issueObject)
    .sort({ name: -1 })
    .exec();
    ...
    await Promise.all(
      dbData.map(async (val, i) => {
        ...
        let obj = {
          ...
          name: val.parking_name ? val.parking_name : "-",
        ...
        if (date1 == date2) {
          ...
          await Promise.all(
            dates.map(async (da, l) => {
              ...
              var doc = await parking_operation
