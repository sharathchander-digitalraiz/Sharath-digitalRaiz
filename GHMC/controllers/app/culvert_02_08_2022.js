...
let formatted_date =   current_datetime.getFullYear() + "-" + (current_datetime.getMonth() + 1)+ "-" +current_datetime.getDate();
 ...
 const culverts_issues_data = await culvertIssueMain.aggregate([
...
exports.culvertissue = async (req, res) =>  
{
  ...
  let formatted_date = new String(current_datetime.getFullYear()) + "-" + new String((current_datetime.getMonth() + 1))+ "-" +cd;
  ...
  console.log("dates");
  console.log(dates);
  var d=await culvertIssueMain.findOne({culvert_id:ObjectId(culvert_id),date:dates}).exec();
  console.log(d);
  const culvertModified=await culvertIssueMain.updateOne({date:dates,culvert_id:ObjectId(culvert_id)},
  {
      ...
      log_date_created:Date.now(),
        },(error,data)=>
        {
          ...
          {
            console.log(data);
            res.status(200).json({success: true,login: true,"message":"Successfully Completed"});
          } 
        })
/*
const cuv_issue = new culvert_issues({
  ...
  */
  ...
  res.status(400).json({success: false,login: true,message:"something went wrongs"});
}
}
...
exports.culvertsolved = async (req, res) =>  
{
  ...
  if(culvert_type_rows.length>0)
  {
    ...
    // current seconds
      ...
      const cuv_issueUpdate = await culvertIssueMain.updateOne({_id:culvert_issue_id}, {
      ...
      if(data) 
      {
        const cdata=await culvertIssueMainfindOne({_id:culvert_issue_id});
        const cdata2=await culvertIssueMain.updateMany({culvert_id:cdata.culvert_id,solved_color:''},
          {solved_date:formatted_date,
          solved_time:time,
          solved_color:'green',
          solved_image:req.file.path,
