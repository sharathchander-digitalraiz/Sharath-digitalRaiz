...
exports.add_culvert = async (req, res) => {
  ...
  const zonelist = await zones
    .findOne({ _id: zone_id }, { _id: 0, id: "$_id", name: 1 })
    .exec();
  const circlelist = await circle
    .findOne({ _id: circle_id }, { _id: 0, id: "$_id", name: 1 })
    .exec();
  const wardslist = await wards
    .findOne({ _id: ward_id }, { _id: 0, id: "$_id", name: 1 })
    .exec();
  const arealist = await area
    .findOne({ _id: areas }, { _id: 0, id: "$_id", name: 1 })
    .exec();
  ...
  cuv.save(async (error, user) => {
    ...
    if (user) {
      ...
      var re = await culvert.updateOne(
        { _id: user._id },
        {
          unique_no: unique_no,
          qr_image:
            "images/qr-images/culvert/templates/" +
            circles +
            "/" +
            unique_no +
            ".png",
...
exports.generate = async (req, res) => {
  ...
  const checkstring = await culvert
...
exports.update_culvert = async (req, res) => {
  ...
  const save_vehicles = await culvert.updateOne(
    { _id: id },
    {
      user_id: user_id,
      zone_id: zone_id,
    ...
    (err, data) => {
      if (err) {
        ...
        responseObject = {
          success: false,
          login: true,
          message: "Sorry unable to update culvert"
        };
        res.status(400).json(responseObject);
      } else if (data) {
        responseObject = {
...
exports.deleteculvert = async (req, res) => {
  ...
  if (data != "0") {
    culvert.findByIdAndRemove({ _id: id }, function (err, docs) {
      if (err) {
        responseObject = {
          success: false,
          login: true,
...
exports.getCulvert = async (req, res) => {
  ...
  if (role_data.name != "admin") {
    const access_data = await useraccess
      .findOne({ _id: acc_dep_data.user_access_id })
      .exec();
...
exports.getallissues_zone_culvert = async (req, res) => {
  ...
  const useraccess_data = await useraccess
    .findOne({ _id: users_data.user_access_id })
    .exec();
  //const culvert_issues_data = await culvert_issues.find({circle_id:useraccess_data.circles}).exec();
  const culverts_issues_data = await culvertIssueMain
    .aggregate([
  ...
  responseObject = {
    ...
    login: true,
    message: "Sucessfully completed",
    data: culverts_issues_data
  };
  res.status(200).json(responseObject);
};

exports.getCulvertissue_type = async (req, res) => {
  const data = await culvert_type
    .find({ status: "Active" }, { _id: 0, name: 1, status: 1, id: "$_id" })
    .exec();
  responseObject = {
    success: false,
    login: true,
...
exports.culvertissue = async (req, res) => {
  try {
    ...
    const culvert_type_data = await culvert_type
      .findOne({ status: "Active", _id: isse_type_id })
      .exec();
    var type = "";
    var check = "";
    var status = "";
    ...
    let formatted_date =
      new String(current_datetime.getFullYear()) + "-" + mn + "-" + cd;
    console.log(formatted_date);
    let dates = formatted_date.toString();
    console.log(culvert_id);
    console.log("culvert_id");
    console.log("dates");
    console.log(dates);
    var d = await culvertIssueMain
      .findOne({ culvert_id: ObjectId(culvert_id), date: dates })
      .exec();
    console.log(d);
    const culvertModified = await culvertIssueMain.updateOne(
      { date: dates, culvert_id: ObjectId(culvert_id) },
      {
...
const cuv_issue = new culvert_issues({
  ...
  */
    ...
    res
      .status(400)
      .json({ success: false, login: true, message: "something went wrongs" });
  }
};
...
exports.culvertsolved = async (req, res) => {
  try {
    ...
    if (culvert_type_rows.length > 0) {
      ...
      const cuv_issueUpdate = await culvertIssueMain.updateOne(
        { _id: culvert_issue_id },
        {
        ...
        async (err, data) => {
          ...
          if (data) {
            const cdata = await culvertIssueMain.findOne({
              _id: culvert_issue_id
            });
            const cdata2 = await culvertIssueMain.updateMany(
              { culvert_id: cdata.culvert_id, solved_color: "" },
              {
                solved_date: formatted_date,
                solved_time: time,
                solved_color: "green",
            ...
            responseObject = {
              success: true,
              login: true,
              message: "Culvert  Solved Submitted Successfully"
            };
            res.status(200).json(responseObject);
          }
        }
      ...
      responseObject = {
        success: false,
        login: true,
        message: "Allow only in location"
      };
      res.status(400).json(responseObject);
    }
  } catch {
