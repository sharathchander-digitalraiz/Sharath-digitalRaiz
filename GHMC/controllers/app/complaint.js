...
{ 
  let services=await servicestypes.find({status:"Active"},{ _id: 0,id:"$_id",name:1}).exec();
  responseObject = {success: true,login: true,message: 'Successfully completed',data:services};
  res.status(200).json(responseObject);
}

exports.add_support = async(req, res) =>  
{ 
    const { user_id,support_list_id,description,zones_id,circles_id} = req.body; 
...
exports.getallcomplaint = async (req, res) => {
    ...
    return res.status(200).json({status:true,message:'Success',data:alldata});   
} 
...
exports.editcomplaint = async (req,res) =>{   
  ...
  exports.updatecomplaint = async(req,res)=>{
    ...
    await support.findByIdAndUpdate({_id:id},{...updateRecords},(err,data)=>{  
        //  console.log(data); 
           if(err){ 
