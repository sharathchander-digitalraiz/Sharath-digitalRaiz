...
const streetVendorOpsModel = require("../../../model/streatVendor_operation");
const Wards = require("../../../model/wards");

// street vendor report
exports.streetVendorResport = async function (req, res) {
  //try {
    ...
    zones_details.map(async (val) => {
      ...
      await Promise.all(
        circle_details.map(async (val) => {
          ...
          const total = await streetVendorOpsModel
            .find({
              circles_id: val._id,
              date: formatted_date
            })
          ...
          count_present = await streetVendorOpsModel
            .find({
              circles_id: val._id,
              date: formatted_date,
              attend: "1"
      ...
      await Promise.all(
        circle_details.map(async (val) => {
          ...
          const total = await streetVendorOpsModel
            ...
            .countDocuments();
          const total_present = await streetVendorOpsModel
            .find({
              circles_id: val._id,
              date: formatted_date,
              attend: 1
...
/************************* street vendor Ward Wise Report *******************************/
exports.streetVendorWardWiseReport = async function (req, res) {
  //try {
  const { user_id, tenent_id, date_search } = req.body;
        ...
        circle_details.map(async (cval, i) => {
          ...
          await Promise.all(
            wardData.map(async (wval) => {
              ...
              count = await streetVendorOpsModel
                .find({
                  ward_id: wval._id,
                  date: formatted_date
                })
