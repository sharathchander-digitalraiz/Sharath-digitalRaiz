...
const commmunityOpsModel = require("../../../model/communityHall_operation");
const Wards = require("../../../model/wards");

// parking report
exports.communityHallResport = async function (req, res) {
  //try {
    ...
    zones_details.map(async (val) => {
      ...
      await Promise.all(
        circle_details.map(async (val) => {
          ...
          const total = await commmunityOpsModel
            .find({
              circles_id: val._id,
              date: formatted_date
            })
          ...
          count_present = await commmunityOpsModel
            .find({
              circles_id: val._id,
              date: formatted_date,
              attend: "1"
      ...
      await Promise.all(
        circle_details.map(async (val) => {
          ...
          const total = await commmunityOpsModel
            ...
            .countDocuments();
          const total_present = await commmunityOpsModel
            .find({
              circles_id: val._id,
              date: formatted_date,
              attend: 1
...
/************************* community hall Ward Wise Report *******************************/
exports.communityHallWardWiseReport = async function (req, res) {
  //try {
  const { user_id, tenent_id, date_search } = req.body;
        ...
        circle_details.map(async (cval) => {
          ...
          await Promise.all(
            wardData.map(async (wval) => {
              ...
              count = await commmunityOpsModel
                .find({
                  ward_id: wval._id,
                  date: formatted_date
                })
