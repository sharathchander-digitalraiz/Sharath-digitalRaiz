...
const parkingOpsModel = require("../../../model/parking_operation");
const Wards = require("../../../model/wards");

// parking report
exports.parkingResport = async function (req, res) {
  //try {
    ...
    zones_details.map(async (val) => {
      ...
      await Promise.all(
        circle_details.map(async (val) => {
          ...
          const total = await parkingOpsModel
            .find({
              circles_id: val._id,
              date: formatted_date
            })
          ...
          count_present = await parkingOpsModel
            .find({
              circles_id: val._id,
              date: formatted_date,
              attend: "1"
      ...
      await Promise.all(
        circle_details.map(async (val) => {
          ...
          const total = await parkingOpsModel
            ...
            .countDocuments();
          const total_present = await parkingOpsModel
            .find({
              circles_id: val._id,
              date: formatted_date,
              attend: 1
...
/************************* parking Ward Wise Report *******************************/
exports.parkingWardWiseReport = async function (req, res) {
  //try {
  const { user_id, tenent_id, date_search } = req.body;
