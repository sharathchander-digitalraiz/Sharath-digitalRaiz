...
const gvpBepOpsModel = require("../../../model/gvpBep_operation");
const Wards = require("../../../model/wards");

// gvp bep report
exports.gvpBepResport = async function (req, res) {
  //try {
    ...
    zones_details.map(async (val) => {
      ...
      await Promise.all(
        circle_details.map(async (val) => {
          ...
          const total = await gvpBepOpsModel
            .find({
              circles_id: val._id,
              type: type,
              date: formatted_date
          ...
          count_present = await gvpBepOpsModel
            .find({
              circles_id: val._id,
              type: type,
              date: formatted_date,
      ...
      await Promise.all(
        circle_details.map(async (val) => {
          ...
          const total = await gvpBepOpsModel
            ...
            .countDocuments();
          const total_present = await gvpBepOpsModel
            .find({
              circles_id: val._id,
              type: type,
              date: formatted_date,
...
/************************* gvp bep Ward Wise Report *******************************/
exports.gvpBepWardWiseReport = async function (req, res) {
  //try {
