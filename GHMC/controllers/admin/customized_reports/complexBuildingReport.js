...
const complexBuildingOpsModel = require("../../../model/complexBuilding_operation");
const Wards = require("../../../model/wards");

// complex buildings report
exports.complexBuildingResport = async function (req, res) {
  //try {
  ...
  switch (role_data.name) {
    case "Admin":
      final_array = await admindata(tenent_id, formatted_date);
      break;
    default:
      final_array = await userdata(acc_dep_data, formatted_date);
      break;
  }
...
async function admindata(tenent_id, formatted_date) {
    ...
    zones_details.map(async (val) => {
      ...
      await Promise.all(
        circle_details.map(async (val) => {
          ...
          const total = await complexBuildingOpsModel
            .find({
              circles_id: val._id,
              date: formatted_date
            })
          ...
          count_present = await complexBuildingOpsModel
            .find({
              circles_id: val._id,
              date: formatted_date,
              attend: "1"
          ...
          cir_obj["total_abscent"] = count - count_present;
...
async function userdata(acc_dep_data, formatted_date) {
      ...
      await Promise.all(
        circle_details.map(async (val) => {
          ...
          const total = await complexBuildingOpsModel
            ...
            .countDocuments();
          const total_present = await complexBuildingOpsModel
            .find({
              circles_id: val._id,
              date: formatted_date,
              attend: 1
...
/************************* complex building Ward Wise Report *******************************/
exports.complexBuildingWardWiseReport = async function (req, res) {
  //try {
  const { user_id, tenent_id, date_search } = req.body;
  ...
  switch (role_data.name) {
    case "Admin":
      final_array = await adminWarddata(tenent_id, formatted_date);
      break;
    default:
      final_array = await userwarddata(acc_dep_data, formatted_date);
      break;
  }
  return res.status(200).send({ login: true, status: true, data: final_array });
  // } catch (err) {
...
async function adminWarddata(tenent_id, formatted_date) {
  const zones_details = await Zones.find({
    tenent_id: ObjectId(tenent_id)
  }).exec();
  console.log(tenent_id);
        ...
        circle_details.map(async (cval, i) => {
          ...
          await Promise.all(
            wardData.map(async (wval) => {
              ...
              count = await complexBuildingOpsModel
                .find({
                  ward_id: wval._id,
                  date: formatted_date
                })
...
async function userwarddata(acc_dep_data, formatted_date) {
  const access_data = await Useraccess.findOne({
    _id: acc_dep_data.user_access_id
  }).exec();
