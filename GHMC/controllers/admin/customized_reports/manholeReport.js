...
const manholeOperations = require("../../../model/manhole_operations");

// manhole tree bus stop report
exports.manholeResport = async function (req, res) {
  //try {
    ...
    zones_details.map(async (val) => {
      ...
      await Promise.all(
        circle_details.map(async (val) => {
          ...
          const total = await manholeOperations
            .find({
              circles_id: val._id,
              type: type,
              date: formatted_date
          ...
          count_present = await manholeOperations
            .find({
              circles_id: val._id,
              type: type,
              date: formatted_date,
      ...
      await Promise.all(
        circle_details.map(async (val) => {
          ...
          const total = await manholeOperations
            ...
            .countDocuments();
          const total_present = await manholeOperations
            .find({
              circles_id: val._id,
              type: type,
              date: formatted_date,
...
/************************* Manhole Ward Wise Report *******************************/
exports.manholeWardWiseReport = async function (req, res) {
  //try {
  const { user_id, tenent_id, date_search, type } = req.body;
