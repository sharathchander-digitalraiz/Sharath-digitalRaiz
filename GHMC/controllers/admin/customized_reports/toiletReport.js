...
const toiletOpsModel = require("../../../model/toilets_operations");
const Wards = require("../../../model/wards");

// toilet report
exports.toiletResport = async function (req, res) {
  //try {
    ...
    zones_details.map(async (val) => {
      ...
      await Promise.all(
        circle_details.map(async (val) => {
          ...
          const total = await toiletOpsModel
            .find({
              circles_id: val._id,
              date: formatted_date
            })
          ...
          count_present = await toiletOpsModel
            .find({
              circles_id: val._id,
              date: formatted_date,
              attend: "1"
      ...
      await Promise.all(
        circle_details.map(async (val) => {
          ...
          const total = await toiletOpsModel
            ...
            .countDocuments();
          const total_present = await toiletOpsModel
            .find({
              circles_id: val._id,
              date: formatted_date,
              attend: 1
...
/************************* toilet Ward Wise Report *******************************/
exports.toiletWardWiseReport = async function (req, res) {
  //try {
  const { user_id, tenent_id, date_search } = req.body;
