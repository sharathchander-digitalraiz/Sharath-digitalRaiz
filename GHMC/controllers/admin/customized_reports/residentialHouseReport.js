...
const residHouseOpsModel = require("../../../model/residentialHouse_operation");
const Wards = require("../../../model/wards");

// residential house report
exports.residHouseResport = async function (req, res) {
  //try {
    ...
    zones_details.map(async (val) => {
      ...
      await Promise.all(
        circle_details.map(async (val) => {
          ...
          const total = await residHouseOpsModel
            .find({
              circles_id: val._id,
              date: formatted_date
            })
          ...
          count_present = await residHouseOpsModel
            .find({
              circles_id: val._id,
              date: formatted_date,
              attend: "1"
      ...
      await Promise.all(
        circle_details.map(async (val) => {
          ...
          const total = await residHouseOpsModel
            ...
            .countDocuments();
          const total_present = await residHouseOpsModel
            .find({
              circles_id: val._id,
              date: formatted_date,
              attend: 1
...
/************************* residential house Ward Wise Report *******************************/
exports.residentialHouseWardWiseReport = async function (req, res) {
  //try {
  const { user_id, tenent_id, date_search } = req.body;
        ...
        circle_details.map(async (cval, i) => {
          ...
          await Promise.all(
            wardData.map(async (wval) => {
              ...
              count = await residHouseOpsModel
                .find({
                  ward_id: wval._id,
                  date: formatted_date
                })
