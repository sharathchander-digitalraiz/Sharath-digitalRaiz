...
const openplaceOpsModel = require("../../../model/openPlace_operation");
const Wards = require("../../../model/wards");

// open place report
exports.openPlaceResport = async function (req, res) {
  //try {
    ...
    zones_details.map(async (val) => {
      ...
      await Promise.all(
        circle_details.map(async (val) => {
          ...
          const total = await openplaceOpsModel
            .find({
              circles_id: val._id,
              date: formatted_date
            })
          ...
          count_present = await openplaceOpsModel
            .find({
              circles_id: val._id,
              date: formatted_date,
              attend: "1"
      ...
      await Promise.all(
        circle_details.map(async (val) => {
          ...
          const total = await openplaceOpsModel
            ...
            .countDocuments();
          const total_present = await openplaceOpsModel
            .find({
              circles_id: val._id,
              date: formatted_date,
              attend: 1
...
/************************* open place Ward Wise Report *******************************/
exports.openPlaceWardWiseReport = async function (req, res) {
  //try {
  const { user_id, tenent_id, date_search } = req.body;
