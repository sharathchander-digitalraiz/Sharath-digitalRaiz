...
exports.circle_transfer_station_attendance_transport = async (req, res) => {
      ...
      zones_details.map(async (val) => {
        ...
        await Promise.all(
          circle_details.map(async (val) => {
            ...
            const total_vehicles_ghmc = await Vehicles_attendance.find({
              owner_type: "GHMC",
              circles_id: val._id,
              vehicle_type: {
            ...
            cir_obj["total_vehicles_ghmc"] = total_vehicles_ghmc;
            cir_obj["total_vehicles_present_ghmc"] =
              total_vehicles_present_ghmc;
            cir_obj["total_vehicles_abscent_ghmc"] =
            ...
            cir_obj["total_ghmc_trips"] = total_trips_ghmc;
            let trips_details_ghmc = await Transfer_station.aggregate([
              {
                $match: { date: formatted_date, circle_id: val._id },
              },
              {
                $group: { _id: 1, TotalAmount: { $sum: "$wastage_weight" } },
              },
            ]);
            //console.log(trips_details_ghmc)
            if (trips_details_ghmc.length > 0) {
              cir_obj["ghmc_weight"] = trips_details_ghmc[0].TotalAmount;
            } else {
              cir_obj["ghmc_weight"] = 0;
            }

            /* PRIVATE */
            const total_vehicles_private = await Vehicles_attendance.find({
              owner_type: "Private",
              circles_id: val._id,
              vehicle_type: {
                $nin: ["GHMC Swatch Auto", "Private Swatch Auto"],
            ...
            cir_obj["total_vehicles_private"] = total_vehicles_private;
            cir_obj["total_vehicles_present_private"] =
              total_vehicles_present_private;
            cir_obj["total_vehicles_abscent_private"] =
            ...
            if (trips_details_private.length > 0) {
              cir_obj["private_weight"] = trips_details_private[0].TotalAmount;
            } else {
              cir_obj["private_weight"] = 0;
            }
