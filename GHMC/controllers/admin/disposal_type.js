...
exports.getalldisposal_type = async (req,res)=>{     
    const alldata = await disposal_type.find({status:'Active'}).exec(); 
    return res.status(200).json({status:true,data:alldata});   
  }  

exports.Tenentalldisposal_type = async (req,res)=>{     
    const alldata = await disposal_type.find({status:'Active',tenent_id:req.body.tenent_id}).exec(); 
    return res.status(200).json({status:true,data:alldata});   
  }  


  exports.editdisposal_type = async (req,res) =>{   
    const id = req.body.id;  
    await disposal_type.findOne({_id:id},(err,data)=>{  
        if(err){ 
  ...
  exports.deletedisposal_type = async (req, res) => { 
    const _id = req.body.id;    
   // console.log(id);
    const updateRecords={ 
      status:'In-active' 
    }
    await disposal_type.findByIdAndUpdate({_id},{...updateRecords},(err,data)=>{  
     //  console.log(data); 
        if(err){ 
  ...
  exports.updatedisposal_type = async(req,res)=>{
    const id = req.body.id;
   // console.log(id); 
    if(!id){
    ...
    await disposal_type.findByIdAndUpdate({_id:id},{...updateRecords},(err,data)=>{  
        //  console.log(data);  
           if(err){ 
