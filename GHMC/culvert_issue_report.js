...
exports.culvert_issue_report_search = async(req,res)=>{ 
       ...
       let role_data       = await Department.findOne({_id : acc_dep_data.department_id},{name:1});
        ...
        { 
            if(role_data.name!='Admin')
            {
                   ...
                   var  circles = access_data['circles'].map((val)=>{
                    return ObjectId(val);
                   })
       ...
       // console.log(role_data.name);
          ...
          //  console.log("hello");
            ...
            // else if(date1!='null')
            // {
            //     Object.date={$gte: date1, $lt: date2};
            // }
           
            const CulvertData = await Culvert.find(issueObject).exec(); 
            ...
            async function processculvert_issue(data){
               ...
               await Promise.all(CulvertData.map( async(val,i)=>{ 
                   ...
                   //console.log(i);   
                    ...
                    {
                        type='today';
                        var obj=await culvertIssueMain.findOne({culvert_id:val._id,date:date1},{}).sort({_id:-1});
                        
                        culvert_obj['visit_date'] = (obj.date!="" && obj.date!=undefined)?obj.date:'-';
                        culvert_obj['visit_image'] =(obj.image!="" && obj.image!=undefined)?obj.image:'-';
                      //  culvert_obj['reported_depth'] = (obj.issue_depth!="" && obj.issue_depth!=undefined)?obj.issue_depth:'-';
                       // culvert_obj['reported_status'] = (obj.status!="" && obj.status!=undefined)?obj.status:'-';
                        culvert_obj['color'] = (obj.type!="" && obj.type!=undefined)?obj.type:'-';
                        culvert_obj['resolved_date']   = (obj.solved_date!="" && obj.solved_date!=undefined)?obj.solved_date:'-';
                        culvert_obj['resolved_image']  = (obj.solved_image!="" && obj.solved_image!=undefined)?obj.solved_image:'-';
                       // culvert_obj['resolved_status'] = (obj.status!="" && obj.status!=undefined)?obj.status:'-';
                       // culvert_obj['resolved_user']   = (obj.user_id!="" && obj.user_id!=undefined)?obj._user_id:'-';
                        culvert_obj['resolved_color'] = (obj.solved_color!="" && obj.solved_color!=undefined)?obj.solved_color:'-';
                        culvert_obj['data']=[];
                        if(i==0)
                        {
                         /*   head.push("visit_date","visit_image","reported_depth","reported_status","color","resolved_date","resolved_image",
                            "resolved_color","resolved_status","resolved_user","resolved_color");
                            */
                    ...
                    else
                    {
                        ...
                        await Promise.all(dates.map( async(da,l)=>{
                          ...
                          //  var df=da;
                            ...
                            var obj=await culvertIssueMain.findOne({culvert_id:val._id,date:df},{}).countDocuments(); 
                            ...
                            if(obj.color=="red" && obj.solved_color=="")
                            {
                                oj.red=1;
                            }
                            else if(obj.color=="red" && obj.solved_color!="")
                            {
                                oj.red=1;
                            }
                            ...
                            // culvert_obj["data"].push({
                            // "red":red,
                            // "orange":orange,
                            // "green":green});

                            culvert_obj["data"].push(oj);
                            // culvert_obj["data"][l]["orange"]=orange;
                            // culvert_obj["data"][l]["green"]=green;
                            console.log(culvert_obj["data"]);
                            console.log(l);
                            }));
             ...
             userToken = processculvert_issue(1);
             
             userToken.then(async function(result) {
                res.status(200).send({login:true,status:true,result:result,"headers":head,'type':type});
            });
}
...
exports.culvertAppCircleIssueSDownload = async(req,res)=>{  
    ...
    'Culvert Name','Culvert Type','Visit date',"Color","Resolved date","Resolved coor"];
    ...
    await Promise.all(CulvertData.map( async(data,index)=>{
        const CulvertIssuesData = await Culvert_issue.find({culvert_id:data._id}).exec(); 
         ...
         "Resolved date":data.solved_date,"Resolved coor":data.solved_color};
...
let s = new String(current_datetime.getDate());
  ...
  else
  {
    ...
    Culvert_rows.forEach(async function(item)
   {
    ...
    var culvert=await culvertIssueMain.findOne({culvert_id:item._id});
     var type=""; 
     var isse_type_id="";
     var issue_name="";
     var issue_depth="";
     var status="";
     var image="";
     if(culvert.type == "red" && culvert.solved_color=="")
    {
        type='red';
        isse_type_id=culvert.isse_type_id;
      //  solved_color=culvert.solved_color;
        issue_name=culvert.issue_name;
        issue_depth=culvert.issue_depth;
        image=culvert.image;
