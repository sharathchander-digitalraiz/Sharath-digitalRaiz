...
exports.plotDetailsReport = async function (req, res) {
  ...
  exports.searchQuery = async (req, res) => {
    try {
      const plotDetails = plotModel.findById({ _id: req.query.id });
      if (plotDetails) {
        res.status(200).json({
          success: true,
  ...
  exports.getPlotBySearch = async (req, res) => {
    try {
      const plotDetails = await plotModel.find({
        $or: [{ plotNumber: { $regex: req.params.key } }],
      });
      if (plotDetails) {
        res.status(200).json({
          success: true,
  ...
  app.get("/:list_name", async (req, res) => {
    try {
      const plotDetails = plotModel(req.params.list_name).find({}).toArray();
      if (plotDetails) {
        res.status(200).json({
          success: true,
          message: "data successfully retrieved. ",
